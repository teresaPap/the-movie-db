...
import { IMovie, ISearchResults, IMovieDisplayData } from '../interfaces';
      ...
      .pipe(
        ...
        map((movies: IMovie[]) =>
          movies.map(({ poster_path, title, vote_average }) => {
            return { poster_path: `https://image.tmdb.org/t/p/w500/${poster_path}`, title, vote_average };
          })
        )
      );
  }
